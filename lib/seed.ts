      ...
      try {
        return client.sql`
          ...
          VALUES (
            ...
            ${parent.password}, 
            ${parent.avatar_img}, 
            ${parent.strip_account_id.length > 0 ? `{${parent.strip_account_id.join(",")}}` : null},  -- Format as array literal
            ${parent.balance.length > 0 ? `{${parent.balance.join(",")}}` : null} -- Format as array literal
          ) 
...
export async function seedChildTable() {
  ...
  await client.sql`
    CREATE TABLE IF NOT EXISTS children (
      ...
      avatar_img VARCHAR(255),
      checking_balance NUMERIC[],  -- Arrays of numbers
      savings_balance NUMERIC[],   -- Arrays of numbers
      pin INT NOT NULL,
